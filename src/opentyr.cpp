...
#include "opentyrian_version.hpp"
#include "params.hpp"
#include "picload.hpp"
...
const char *opentyrian_version = OPENTYRIAN_VERSION;
	...
	MenuOptions& operator++(MenuOptions& a)
    {
      // prefix
      a = (MenuOptions)(a + 1);
      return a;
    }
    MenuOptions operator++(MenuOptions& a, int)
    {
      // postfix
      auto prev = a;
      a         = (MenuOptions)(a + 1);
...
void opentyrian_menu( void )
{
	...
	do
	{
		...
		for (MenuOptions i = (MenuOptions)0; i < MenuOptions_MAX; i++)
		{
		...
		if (newkey)
		{
			...
			case SDL_SCANCODE_UP:
				...
				{
					if (sel-- == 0)
						sel = (MenuOptions)(MenuOptions_MAX - 1);
				}
			...
			case SDL_SCANCODE_RETURN:
				...
				case MENU_SCALING_MODE:
					JE_playSampleNum(S_SELECT);
					scaling_mode = (ScalingMode)temp_scaling_mode;
					break;
